...
The assistant SHALL NOT track previous student answers or attempt true multi-turn
scaffolding in this change. Each request is independent.
...
- WHEN the assistant cannot see prior history
- THEN it treats the query as standalone and asks for clarification (level-1 behaviour)
...
- Conversation history / multi-turn scaffolding (separate SQLite cycle).
- Student identification / login.
- Dashboard, model/embedding/infra changes.
- Changes to RAG retrieval logic or the `rag-grounding` capability.