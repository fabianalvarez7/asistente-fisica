...
> Source: `openspec/changes/deploy-hf-spaces/specs/hf-spaces-deploy/spec.md`
...
This spec was verified against 11 commits on `feat/deploy-hf-spaces`. 8 SHALLs implemented: 6 PASS, 2 PARTIAL.

**PARTIAL SHALLs as archived:**
- **HS-S8 (Local Dev Parity)**: PARTIAL because `.env.example` sets `CHROMA_PERSIST_DIR=./data/chroma` (gitignored), while the committed bake lives at `rag/index/chroma/`. Clean-clone local dev requires either fixing `.env.example` to `./rag/index/chroma` or an explicit copy step. See WARNING W1 in the archive report.
- **HS-S5 (Sleep/Wake Resilience)**: PARTIAL by necessity — requires live HF Space deploy to verify. Guaranteed by design (artifacts in git, not ephemeral disk).

**Deviation from design**: T0 changed chunk_size from 1000 to 500 (design OQ-C2 resolved "keep 1000"). Required to meet the >25 chunk threshold (1000 yields ~15-16 chunks on the cuadernillo). Documented in apply-progress and verify-report.

## References

- `openspec/changes/deploy-hf-spaces/archive-report.md`
- `openspec/changes/deploy-hf-spaces/specs/hf-spaces-deploy/spec.md` (change-specific source)
- `openspec/specs/corpus-rebake/spec.md` (companion capability)
